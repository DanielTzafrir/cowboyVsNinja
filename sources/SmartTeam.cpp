...
namespace ariel {
	...
	// This function return the idx of the most dangerus enemy
    	int SmartTeam::chooseVictim(Character* leader) {
		int victimIdx = -1;
		for (int i = 0; i < getVecSize(); i++) {
			if (getFighter(i)->getIsCowboy() && getFighter(i)->isAlive()) {
				victimIdx = i;
		...
		for (int i = 0; i < getVecSize(); i++) {
			if (!getFighter(i)->getIsCowboy() && getFighter(i)->isAlive() && getFighter(i)->getSpeed() == 8) {
				victimIdx = i;
			}
		}
		for (int i = 0; i < getVecSize(); i++) {
			if (!getFighter(i)->getIsCowboy() && getFighter(i)->isAlive() && getFighter(i)->getSpeed() == 12) {
				victimIdx = i;
			}
		}
		for (int i = 0; i < getVecSize(); i++) {
			if (!getFighter(i)->getIsCowboy() && getFighter(i)->isAlive() && getFighter(i)->getSpeed() == 14) {
				victimIdx = i;
			}
		}
		return victimIdx;
